...
const outcomeColors = ["#10b981", "#ef4444", "#3b82f6", "#f59e0b", "#8b5cf6", "#ec4899"];
...
function parseMarket(market) {
  ...
  const isBinary = outcomes.length === 2 && (outcomes[0] === "Yes" || outcomes[0] === "YES");
  const labels = isBinary ? ["OUI", "NON"] : outcomes;
  return { labels, prices, isBinary };
}

function MarketCard({ market, onRemove, prevPrices }) {
  const { labels, prices } = parseMarket(market);
  const cat = market.category || "default";
  const catColor = categoryColors[cat] || categoryColors.default;

  const prevFirst = prevPrices?.[market.id]?.prices?.[0];
  const currentFirst = formatPct(prices[0]);
  const diff = prevFirst !== undefined ? currentFirst - prevFirst : 0;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {labels.map((label, i) => {
          const pct = formatPct(prices[i]);
          const color = outcomeColors[i % outcomeColors.length];
          return (
            <div key={i}>
    ...
    >
      ...
      >
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "8px 12px" }}>
          ...
          {results.map((m) => {
            const { labels, prices } = parseMarket(m);
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, marginTop: 2 }}>
                    {m.category || "General"} · {labels.length} option{labels.length > 1 ? "s" : ""}
                  </div>
                </div>
                <div style={{ textAlign: "right", minWidth: 40 }}>
                  <div style={{ color: outcomeColors[0], fontWeight: 700, fontSize: 15, fontFamily: "monospace" }}>
                    {formatPct(prices[0])}%
                  </div>
                  <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10 }}>{labels[0]}</div>
                </div>
  ...
  const refreshPrices = useCallback(async () => {
    ...
    try {
      ...
      watched.forEach((m) => {
        ...
        prev[m.id] = { prices: prices.map((p) => formatPct(p)) };
      });
      setPrevPrices(prev);
      setWatched(updated);
      ...
      <div style={{ padding: "32px" }}>
        {watched.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ fontSize: 14, color: "rgba(255,255,255,0.35)", maxWidth: 340, lineHeight: 1.6 }}>
              Cliquez sur <strong>+ Ajouter un marché</strong> pour rechercher et suivre des paris Polymarket en temps réel.
            </div>
            <button
              onClick={() => setShowSearch(true)}
              style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
            {watched.map((m) => (
              <MarketCard key={m.id} market={m} onRemove={removeMarket} prevPrices={prevPrices} />
            ))}
          </div>
        )}
      </div>
