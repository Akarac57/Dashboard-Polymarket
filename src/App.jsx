...
// Un event contient plusieurs markets binaires (un par candidat/option)
// On prend le prix "Yes" de chaque market comme la probabilité de cet outcome
function parseEvent(event) {
  const markets = event.markets || [];

  // Cas 1 : event avec plusieurs markets → chaque market = une option (ex: Démocrate, Républicain)
  if (markets.length > 1) {
    return markets.map((m) => {
      ...
      return {
        label,
        pct: formatPct(prices[0]), // prix "Yes" = probabilité de cet outcome
      };
    }).sort((a, b) => b.pct - a.pct);
  }

  // Cas 2 : event avec un seul market binaire (Oui/Non)
  if (markets.length === 1) {
    const m = markets[0];
    ...
    return [
      { label: isBinary ? "OUI" : outcomes[0], pct: formatPct(prices[0]) },
      { label: isBinary ? "NON" : outcomes[1], pct: formatPct(prices[1]) },
    ];
  }
...
function SearchModal({ onClose, onAdd, watchedIds }) {
  ...
  const search = useCallback(async (q) => {
    ...
    try {
      ...
      // On prend directement les events
  ...
  // Tendances au chargement : events triés par volume
  useEffect(() => {
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, marginTop: 2 }}>
                    {event.category || "General"} · {markets.length} option{markets.length > 1 ? "s" : ""}
                  </div>
                </div>
                {firstPct !== null && (
...
export default function PolymarketDashboard() {
  ...
  const refreshPrices = useCallback(async () => {
    ...
    try {
      ...
      // Sauvegarder les prix actuels avant mise à jour
      const prev = {};
      watched.forEach((ev) => {
