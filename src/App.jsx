...
// Normalise la catégorie brute Polymarket en clé simple
function normalizeCat(raw = "") {
  const s = raw.toLowerCase().trim();
  if (!s || s === "default") return "default";
  if (s.includes("polit") || s.includes("election") || s.includes("government")) return "politics";
  if (s.includes("crypto") || s.includes("bitcoin") || s.includes("ethereum") || s.includes("defi") || s.includes("web3")) return "crypto";
  if (s.includes("sport") || s.includes("soccer") || s.includes("football") || s.includes("nba") || s.includes("nfl") || s.includes("tennis") || s.includes("golf") || s.includes("baseball") || s.includes("hockey")) return "sports";
  if (s.includes("science") || s.includes("ai") || s.includes("tech")) return "science";
  if (s.includes("business") || s.includes("finance") || s.includes("economy") || s.includes("market")) return "business";
  if (s.includes("entertain") || s.includes("culture") || s.includes("pop") || s.includes("music") || s.includes("movie")) return "entertainment";
  return s;
}
...
function getEventTab(event) {
  const cat = normalizeCat(event.category || "");
  if (["politics","crypto","sports","science","business"].includes(cat)) return cat;
  return "all";
}
